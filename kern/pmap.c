...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	//boot_map_region_large(kern_pgdir, KERNBASE, ~KERNBASE + 1, 0, PTE_W);
	...
	// put the PSE to turn on the big page
	uint32_t cr4 = rcr4();
	cr4 |= CR4_PSE;
	lcr4(cr4);
...
mem_init_mp(void)
{
	...
	for(int i = 0; i < NCPU; i++){
		boot_map_region(kern_pgdir, KSTACKTOP - i * (KSTKGAP + KSTKSIZE) - KSTKSIZE, KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W);
	}
}
...
page_init(void)
{
	...
	size_t ext_num = EXTPHYSMEM / PGSIZE;
	for(; i < ext_num; i++){
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}

	size_t next_num = PGNUM(PADDR(boot_alloc(0)));
	for(; i < next_num; i++){
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}
...
page_free(struct PageInfo *pp)
{
	...
	/*if(pp->pp_ref != 0 || pp->pp_link != NULL)
		panic("pp_ref or pp_link is not NULL\n*/
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(!(*va_dir & PTE_P)){
		...
		new_page->pp_ref++;
		*va_dir = page2pa(new_page) | PTE_P | PTE_W | PTE_U;
	}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for(int i = 0; i < len; i++){
		...
		*tmp_pte = (pa + i * PGSIZE) | perm | PTE_P;
	}
}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	struct PageInfo *tmp_page = page_lookup(pgdir, va, &pte_store);

	if(tmp_page == NULL) return; // do noting
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for(uintptr_t va_i = (uintptr_t)va; va_i < end; va_i = ROUNDDOWN(va_i + PGSIZE, PGSIZE)){
		...
		pte_t* tmp_pte = pgdir_walk(env->env_pgdir, (void*)va_i, 0);
		if(!tmp_pte || (*tmp_pte & perm) != perm){
			user_mem_check_addr = va_i;
			return -E_FAULT;
		}
	}
