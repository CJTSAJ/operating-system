...
trap_init(void)
{
  ...
  extern void ENTRY_IRQ_ERROR   ();// 19
	...
	SETGATE(idt[T_BRKPT  ],0,GD_KT,ENTRY_BRKPT  ,3); // one byte INT 3 instruction
	SETGATE(idt[T_OFLOW  ],0,GD_KT,ENTRY_OFLOW  ,3); //INT0 instruction
	SETGATE(idt[T_BOUND  ],0,GD_KT,ENTRY_BOUND  ,3); //BOUND instruction
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if(curenv->env_pgfault_upcall){
		...
		if(UXSTACKTOP - tf->tf_esp < PGSIZE) //already user exception stack
